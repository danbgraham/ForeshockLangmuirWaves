...
WmaxSW = Wmax(idxSW);
EmaxSW = Emax(idxSW);
Temedian = Temedian(idxSW);
nemedian = nemedian(idxSW);
starttimesSW = starttimesall(idxSW);
...
Wc = eps0*(10.^logEcES*1e-3).^2./(4*neES.*TeES*qe.*1e6);
...
[PWc,xWc] = hist_dg(log10(Wc),'range',[-6.5 -1.5],'nbins',100);
...
xwidth = 0.42;
ywidth = 0.78;
set(h(1),'position',[0.07 0.19 xwidth ywidth]);
set(h(2),'position',[0.57 0.19 xwidth ywidth]);
...
irf_legend(h(1),'W_{max}',[0.8 0.99],'color','k','fontsize',16)
irf_legend(h(1),'W_{c}',[0.76 0.90],'color','r','fontsize',16)
irf_legend(h(1),'(a)',[0.99 0.99],'color','k','fontsize',16)
...
hold(h(2),'off')
irf_legend(h(2),'E_{max}',[0.8 0.99],'color','k','fontsize',16)
irf_legend(h(2),'E_{c}',[0.76 0.90],'color','r','fontsize',16)
axis(h(2),[5 300 1 2e3])
xlabel(h(2),'E_{max} (mV m^{-1})')
ylabel(h(2),'P(log E_{max})')
irf_legend(h(2),'(b)',[0.99 0.99],'color','k','fontsize',16)

set(h(1:2),'fontsize',16)